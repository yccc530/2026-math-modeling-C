...
def audit_project() -> Dict[str, object]:
    ...
    if "全局最优" in report_text and "不声称" not in report_text:
        issues.append("Technical report may overclaim global optimality.")
    if "return_to_depot" not in report_text:
        issues.append("Technical report does not explain return_to_depot.")
    if "启发式" not in report_text:
        warnings.append("Technical report should explicitly discuss heuristic limitations.")

    status = "PASS" if not issues else "FAIL"
    audit_md = REPORTS_DIR / "audit_report.md"
    bug_md = REPORTS_DIR / "bug_list.md"
    fix_md = REPORTS_DIR / "fix_plan.md"
    audit_lines = [f"# Audit Report\n\nStatus: {status}\n"]
    if issues:
        audit_lines.append("## Issues\n")
        audit_lines.extend(f"- {issue}\n" for issue in issues)
    else:
        audit_lines.append("All hard constraints, required files and validation reports passed the audit.\n")
    if warnings:
        audit_lines.append("\n## Warnings\n")
        audit_lines.extend(f"- {w}\n" for w in warnings)
    audit_md.write_text("".join(audit_lines), encoding="utf-8")

    if issues:
        bug_md.write_text("# Bug List\n\n" + "".join(f"- {issue}\n" for issue in issues), encoding="utf-8")
        fix_md.write_text(
            "# Fix Plan\n\n"
            "1. Re-run `python main.py` to regenerate failed artifacts.\n"
            "2. Inspect validation JSON files for item-level violations.\n"
            "3. If failures persist, reduce route cluster size or split overloaded station groups.\n",
            encoding="utf-8",
        )
    else:
        bug_md.write_text("# Bug List\n\nNo unresolved bugs found.\n", encoding="utf-8")
        fix_md.write_text("# Fix Plan\n\nNo automatic fixes required.\n", encoding="utf-8")
