...
def solve_q1(iteration: int = 0) -> Dict[str, object]:
    ...
    # Q1 的一级目标是装入件数。完整类别优先多 seed 搜索用于寻找当前最优可行解；
    # 其他排序用于补充 Pareto 记录。
...
def _write_q1_optimization_artifacts(records: List[Dict[str, object]], best_meta: Dict[str, object], truck) -> None:
    ...
    report.write_text(
        ...
        f"可行起点评估数量：{len(records)}。\n\n"
        f"当前最优策略：{best_meta.get('strategy')}，随机种子为 {best_meta.get('seed')}。\n\n"
        f"当前最优目标元组：`{best_meta.get('objective_tuple')}`。\n\n"
        f"宽松件数上界：{bounds['relaxed_count_upper']}；当前装入件数：{best_count}；差距：{bounds['relaxed_count_upper'] - best_count} 件。\n\n"
        f"宽松体积上界：{bounds['volume_upper_m3']:.6f} m3；当前装入体积：{best_volume:.6f} m3；"
        f"差距：{max(0.0, bounds['volume_upper_m3'] - best_volume):.6f} m3。\n\n"
        f"类别 I 落地面积上界：{bounds['category_i_floor_area_upper']} 件；类别 II 顶层面积上界：{bounds['category_ii_top_area_upper']} 件。\n\n"
